# Salve App
Accessibility Communication Platform for Blind and Deaf-Blind Users
## Accessible Communication for Every Heart

Salve App is an accessibility-focused communication platform designed to help blind and deaf-blind individuals communicate more independently with their families through voice commands, gesture navigation, and haptic feedback.

This project was inspired by my blind daughter and was created to explore how modern AI-assisted development can improve accessibility and communication for people with visual impairments.

## Project Status

✅ Functional Web Platform

🚧 Mobile Application Version Under Development

📈 Approximately 80% Complete

## Live Demo

Website:

https://salveapp-ztvyz8sv.manus.space/website/

## Key Features

* Voice Command Interaction
* Gesture-Based Navigation
* Haptic Feedback
* Voice Messaging
* Voice & Video Calling Concepts
* Parent Communication Interface
* Accessibility-First Design
* No-Look User Experience

## Technologies & Tools

* Manus AI
* ChatGPT
* Claude
* Prompt Engineering
* AI-Assisted Product Development

## Project Goals

* Improve communication accessibility
* Support blind and deaf-blind users
* Demonstrate AI-assisted software development
* Create meaningful real-world solutions

## My Role

Founder • Product Designer • AI Builder

## Author

Ramil V. Natan

Technical Enablement Specialist | Engineering Educator | AI Builder
